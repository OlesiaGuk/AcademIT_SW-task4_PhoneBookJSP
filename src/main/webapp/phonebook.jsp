    ...
    <form method="post" action="delete">
        <table class="table table-bordered contact-table">
            ...
            <tr>
                <th>
                    <label class="select-all-label">
                        <input type="checkbox" title="Выбрать" name="checkAll" onchange="checkedAll(this)"/>
                    </label>
                </th>
                <th>№</th>
                <th>Фамилия</th>
            ...
            <tr>
                ...
                <td>
                    <button type="submit" class='btn btn-primary' name="oneContactId" value="<%=contact.getId()%>">
                        Удалить
                    </button>
                </td>
            </tr>
...
<script type="text/javascript">
    function checkedAll(checkAllCheckbox) {
        let allCheckBoxes = document.getElementsByClassName("select-me");
        let checkState = !!checkAllCheckbox.checked;

        if (allCheckBoxes.length > 0) {
            for (let i = 0; i < allCheckBoxes.length; i++) {
                allCheckBoxes[i].checked = checkState;
            }
        }
    }
</script>
